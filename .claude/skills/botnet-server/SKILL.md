...
- **A new synced entity is five edits, and the fifth is the one that bites.** Adding `events` needed: the table + `chg_<entity>_*` triggers in `migrate`, a `ChangedIDs` field, a `bucket()` case, and — easy to miss — the `sortAll` slice AND `emptyChangedIDs`. A bucket left out of `emptyChangedIDs` marshals as `null` instead of `[]`, which the client has no nil case for; a bucket left out of `sortAll` leaks map iteration order into the API. Grep the entity name across `changes.go` and expect five hits.
- **Storing a time you will RANGE-QUERY as text? Fixed width or nothing.** The store's `fmtTime` is `RFC3339Nano`, which drops trailing zeros from the fraction — so `12:00:00Z` sorts AFTER `12:00:00.5Z`, and any `WHERE starts_at > ?` silently returns the wrong rows. Events store `t.UTC().Truncate(time.Second).Format(time.RFC3339)` instead. The bug is invisible until someone stores a sub-second time; write the test that does.
- **Adding a tool renumbers every positional assertion in the suite.** `toolWireDefs` is indexed by position in seven existing tests (`tools_test.go`, `websearch_test.go`, `search_test.go`), and adding `calendar` broke all of them. When you touch that array, expect the churn, and prefer `payload.Tools[len(payload.Tools)-1]` for the search tool — the store-backed tools go first, search is always last.
- **Anything added to `promptContext` is added to every prompt-shape test too.** The unconditional date-time line moved four "request carried N turns" assertions. That is the cost of an always-present block; it is still the right shape for ground truth like the current time, which a model cannot be trusted to have.
